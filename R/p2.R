...
pic <- pich21 %>% group_by(`Canton Domic`) %>% dplyr::mutate(Count = n()) %>% group_by(`Canton Domic`, SE)%>% summarize_at(vars(`confirmados`), funs(sum))
